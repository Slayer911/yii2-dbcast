...
--
-- Описание для таблицы module_banner
--
DROP TABLE IF EXISTS module_banner;
CREATE TABLE module_banner (
  id INT(11) NOT NULL AUTO_INCREMENT,
  title VARCHAR(255) NOT NULL COMMENT 'Название|text',
  is_active TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Активность|checkbox',
  image VARCHAR(255) DEFAULT NULL COMMENT 'Картинка|image',
  url VARCHAR(255) DEFAULT NULL COMMENT 'Ссылка|text',
  rang SMALLINT(6) DEFAULT 0 COMMENT 'Ранг|int',
  time_end INT(11) DEFAULT NULL COMMENT 'Конечное время отображения|datetime',
  PRIMARY KEY (id)
)
ENGINE = INNODB
AUTO_INCREMENT = 92
...
CREATE TABLE module_digest (
  ...
  date_out INT(11) NOT NULL DEFAULT 0 COMMENT 'Дата окончания|date',
  ...
  PRIMARY KEY (id),
  INDEX IDX_module_digest (date_in, date_out),
  INDEX IDX_module_digest_date_in (date_in),
  INDEX IDX_module_digest_date_out (date_out)
)
ENGINE = INNODB
AUTO_INCREMENT = 189
AVG_ROW_LENGTH = 1352
...
CREATE TABLE module_event (
  ...
  CONSTRAINT FK_module_event_module_event_id FOREIGN KEY (parent_event_id)
...
--
-- Описание для таблицы module_event_comment
--
DROP TABLE IF EXISTS module_event_comment;
CREATE TABLE module_event_comment (
  id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  event_id INT(11) NOT NULL COMMENT 'Cобытие|select',
  parent_comment_id INT(11) UNSIGNED DEFAULT NULL COMMENT 'Родительский комментарий|select',
  user_id INT(11) NOT NULL COMMENT 'Пользователь|select',
  comment_text TEXT NOT NULL COMMENT 'Комментарий|textarea',
  create_time INT(11) DEFAULT NULL COMMENT 'Время создания|datetime',
  status TINYINT(1) DEFAULT 0 COMMENT 'Статус|select',
  rating TINYINT(1) DEFAULT 0 COMMENT 'Рейтинг|radiolist',
  PRIMARY KEY (id),
  INDEX event_id (event_id),
  INDEX parent_comment_id (parent_comment_id),
  INDEX user_id (user_id),
  CONSTRAINT module_event_comment_ibfk_1 FOREIGN KEY (event_id)
    REFERENCES module_event(id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT module_event_comment_ibfk_2 FOREIGN KEY (parent_comment_id)
    REFERENCES module_event_comment(id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT module_event_comment_ibfk_3 FOREIGN KEY (user_id)
    REFERENCES module_user(id) ON DELETE RESTRICT ON UPDATE CASCADE
)
ENGINE = INNODB
AUTO_INCREMENT = 695
AVG_ROW_LENGTH = 420
...
CREATE TABLE module_event_date (
  id INT(11) NOT NULL AUTO_INCREMENT,
  date_in INT(11) NOT NULL COMMENT 'Дата начала|date',
  ...
  PRIMARY KEY (id),
  INDEX IDX_module_event_vs_date_date_in (date_in),
...
--
-- Описание для таблицы module_event_vs_participant
--
DROP TABLE IF EXISTS module_event_vs_participant;
CREATE TABLE module_event_vs_participant (
  event_id INT(11) NOT NULL COMMENT 'Событие|select',
  participant_id INT(11) NOT NULL COMMENT 'Участник|select',
  type INT(2) DEFAULT 1 COMMENT 'Тип|select',
  create_time INT(11) DEFAULT NULL COMMENT 'Дата создания|datetime',
  PRIMARY KEY (event_id, participant_id),
  INDEX IDX_module_event_vs_user_event_id (event_id),
  INDEX IDX_module_event_vs_user_user_id (participant_id),
  UNIQUE INDEX UK_module_event_vs_user (event_id, participant_id),
  CONSTRAINT FK_module_event_vs_participant_module_event_id FOREIGN KEY (event_id)
    REFERENCES module_event(id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT FK_module_event_vs_participant_module_user_id FOREIGN KEY (participant_id)
    REFERENCES module_user(id) ON DELETE CASCADE ON UPDATE CASCADE
)
ENGINE = INNODB
AVG_ROW_LENGTH = 40
CHARACTER SET utf8
COLLATE utf8_general_ci
COMMENT = 'Связь события и пользователя';
...
CREATE TABLE module_event_vs_tag (
  ...
  CONSTRAINT FK_module_event_vs_tag_module_tag_id FOREIGN KEY (tag_id)
    REFERENCES module_tag(id) ON DELETE CASCADE ON UPDATE NO ACTION
)
ENGINE = INNODB
AVG_ROW_LENGTH = 34
CHARACTER SET utf8
